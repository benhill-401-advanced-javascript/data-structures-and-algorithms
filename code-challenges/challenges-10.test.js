...
const count = (target, input) => {
  // Solution code here...
  return input.map(arr => arr.filter(val => val === target)).reduce((acc, arr) => acc += arr.length, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(arr => arr.filter(val => (typeof val === 'number' && val % 5 === 0))).map(arr => arr.map(val => Math.pow(2, val)));
};
